...
$(document).ready(function () {
  ...
  // Form Submit
  var ltrsForm = $("#ltrs-application-form");
  ltrsForm.on("submit", function (e) {
    ...
    if (ltrsForm.data("otpstate") == 1) {
      return toggleOTPModal();
    }

    if (($("#gen_otp").val() == "0" && $("#input_otp").val() == "") || ltrsForm.data("formstate") == 1) {
      console.log('nani');
      return sendOTP($("#mobile_no").val(), $(this).data('otpstate'), $("#ltrs-application-form"), "ltrs");
    }
  ...
  // btn confrim otp
  $("#btn-confirmotp-ltrs").click(function () {
    if ($('#input_otp').val() == $('#gen_otp').val()) {
      var formData = new FormData($("#ltrs-application-form")[0]);
      return formDataLtrs(formData);
    }
...
function formDataLtrs(formData = null) {
  if (formData === null) {
  ...
  if ($("#ltrs-application-form").data("otp-exp-min") == 0) {
    return false;
  }

  var elem = $("#ltrs-application-form").find('.cus_psgc');
  ...
  return $.ajax({
    ...
    success: function (r) {
      ...
      if (str == '1' || str == 1) {
        ...
        $("#ltrs-application-form").trigger("reset");
        $(".use-select2").val(null).trigger("change");
        $(".cus_psgc").val(null).trigger("change");
        $("#input_otp").val('');
        $("#ltrs-application-form").data("formstate", 0).data("reotp", 0).data("otpstate", 0).data("otptries", 3);
...
$('#yes').on('click', function (){
  var label = "<i style='font-size:12px;color:gray;'>Please enter your Customer Acct No. below to avail free LTRS</i>"
  var input = "<input type='text' name='Account' class='form-control validate' id='Account' required>"
  document.getElementById("new-field").innerHTML = " <div class='form-group col-lg'>"+label+"<br><label for='Account'><font color='red'>* </font>Customer Account No : </label> <br>"+" " + input +"</div>";
})
$('#no').on('click', function (){
  // var input = "<input type='text' >"
  document.getElementById("new-field").innerHTML = " ";
})